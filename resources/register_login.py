from main import api, Resource, fields, jsonify, create_access_token, jwt_required, get_jwt_identity, create_refresh_token, jwt_refresh_token_required, get_raw_jwt, BLACKLIST
from models.user import UserModel, user_schema

register_namespace = api.namespace('register', description='new to our API? kidnly register here')
refresh_namespace = api.namespace('refresh', description='Refreshes user access token after a while')
...
class UserLogin(Resource):
    ...
    def post(self):
        ...
        data = api.payload
        email = data['email']
        if UserModel.check_email(email) and UserModel.authenticate_password(email, data['password']):
